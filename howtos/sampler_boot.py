...
class SamplerBoot:
    ...
    valid_certificate_authority = 'root-ca_rsa'
    CLIENT_KEY_FILE = 'ccsmpClient.key'
...
class SamplerUtil:
    ...
    @staticmethod
    def get_valid_client_certificate(invalid=False) -> str:
        """method to get valid client certificate
        Returns:
            a valid file path for the client certificate
        """
        valid_client_certificate_file_full_path = os.path.join(SamplerUtil.get_trusted_store_dir(invalid),
                                                               SolaceConstants.CLIENT_CERTIFICATE_FILE)

        return valid_client_certificate_file_full_path

    @staticmethod
    def get_valid_client_key(invalid=False) -> str:
        """method to get valid client certificate
        Returns:
            a valid file path for the client certificate
        """
        valid_client_key_file_full_path = os.path.join(SamplerUtil.get_trusted_store_dir(invalid),
                                                       SamplerBoot.CLIENT_KEY_FILE)
    ...
    def cert_feature(semp_props, broker_props, is_enable=True):
        ...
        if is_enable:
            ...
            valid_server_certificate_file = os.path.join(SamplerUtil.get_trusted_store_dir(), 'root_ca-rsa.crt')
    ...
    @staticmethod
    def publisher_terminate(publisher, raise_exception=False):
        try:
...
class SolaceConstants:
    ...
    CLIENT_CERTIFICATE_FILE = 'client-rsa.pem'
    KEY_STORE_PASSWORD = "solace1"
